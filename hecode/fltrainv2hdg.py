...
def create_sequences(data, sequence_length):
    ...
    for i in range(len(data) - sequence_length):
        ...
        targets.append(data[i+sequence_length, 2])  # Use index 3 for hdg instead of 0 for speed
...
def train_client(data_loader, model, criterion, optimizer, grad_scaler, num_epochs):
    model.train()
...
client_data_paths = [
    'D:/ml/perflm/hedata0903/vehicle_1.txt', 
...
for round in range(num_rounds):
    ...
    for path in client_data_paths:
        ...
        client_state = train_client(data_loader, client_model, criterion, optimizer, grad_scaler, num_epochs)
        client_states.append(client_state)

    federated_averaging(global_model, client_states)

    # Save the global model after each round
    # torch.save(global_model.state_dict(), f'D:/ml/perflm/modelhdg/hemodel0903/global_model_round_{round+1}.pth')
